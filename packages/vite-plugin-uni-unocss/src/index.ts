import fs from "fs";
import miniRules from "./mini-rules";
import attributifyRules from "./attributify-rules";
import { cr1, cr2, cr3, cr4, cr5 } from "./css-rules";

// 默认是unocss
let source = "unocss";

function uniTwuCssPlugin(options: {
  source: "unocss" | "windicss" | "tailwindcss";
}) {
  let outDir = "";
...
async function updateWXCss(css) {
  ...
  css = css.replace(cr3, (mItem) => {
    mItem = mItem.replace(cr4, (mItem1) => {
            ...
            return splitSpa
              .map((itemSpa) => {
                for (let i = 0; i < miniRules.length; i++) {
                  ...
                  return itemSpa.trim().replace(miniRules[i], (mItem2) => {
...
async function rewriteStyleCss(css) {
  ...
  css = css.replace(
    /\.[a-zA-Z0-9]*>\s*:not\(\[hidden]\)\s*~\s*:not\(\[hidden]\){.*?}/g,
    ""
  );
...
export default uniTwuCssPlugin;
